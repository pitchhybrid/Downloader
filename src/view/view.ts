...
import { sizeOf, file, toMatrix } from './../utils/utils';
...
export abstract class Abstract {
    ...
    public abstract queryLinksImages(): void;
    ...
    public addList(image: Image, progress: Tampermonkey.ProgressResponseBase): void {
        ...
        var { percent, speed, downloaded } = this.info(image, progress);
        cell.innerHTML = `
    ...
    public addErrorList(image: Image): void {
        const cell: HTMLElement = image.getCell() // li;
        cell.title = image.src;
        cell.onclick = (ev: MouseEvent) => {
            if (!image.done)
        ...
        cell.innerHTML = `
            ...
            <p><small style="float:left; color: #ca2222;">Error:. Unstable Server</small><small style="float:right">retry:. ${image.retry}</small></p>`;
    ...
    private info(image: Image, progress: Tampermonkey.ProgressResponseBase): Progress {

        image.speed.push(progress.loaded);
        ...
        data.speed = image.speed.length > 10 ? `${sizeOf(progress.loaded - image.speed[image.speed.length - 10])}/s` : `${sizeOf(progress.loaded)}/s`;
...
//     var images:Promise<Blob>[][] = toMatrix(this.images.map((image:Image) => {
//         return Object.assign(this.download<Blob>(
//             image.src, {
...
//     console.log((image as any).image)
// }
            ...
            if (!this.images[i].done) {
                do {
                    try {
                        ...
                        this.addErrorList(this.images[i]);
                        this.images[i].done = false;
                        console.error(error)
                        this.images[i].retry++
                    }
                } while (!this.images[i].done && this.images[i].retry <= 5);
            }
        }
    }
    ...
    public download<T>(url: string,
        ...
        return new Promise((resolve: (value: T | PromiseLike<T>) => void, reject: (reason?: any) => void): void => {
            GM_xmlhttpRequest<T>({
                method, url, responseType, timeout,
                onprogress,
                revalidate: true,
